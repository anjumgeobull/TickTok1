...
public class UploadVideoSP {
    private  final UploadVideoService uploadVideoService;

    public UploadVideoSP (Context context) {
        uploadVideoService = APIServiceFactory.createService(UploadVideoService.class, context);
    }

    public  void callUploadVideo(RequestBody user_id, RequestBody visibility, RequestBody video_caption, MultipartBody.Part video, final APICallback apiCallback)
    {
        Call<UploadVideoModel> call = null;
        call = uploadVideoService.uploadVideo(user_id,visibility,video_caption,video);
